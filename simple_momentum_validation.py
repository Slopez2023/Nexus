#!/usr/bin/env python3
"""
Phase 2: Simple Momentum Strategy Validation - Working Implementation

Demonstrates momentum strategy validation with statistical rigor.
Uses the academic Jegadeesh-Titman approach with proper backtesting.
"""
...
def momentum_strategy(returns, formation_days=126):
    ...
    Returns:
        Strategy signal series (-1, 0, 1)
    """
    signals = pd.Series(0, index=returns.index)

    for i in range(formation_days, len(returns)):
        # Formation period cumulative return
        formation_returns = returns.iloc[i-formation_days:i]
        cum_return = (1 + formation_returns).prod() - 1

        # JT Momentum: Buy winners, short losers
        if cum_return > 0.05:  # 5%+ gain = long
            signals.iloc[i] = 1
        elif cum_return < -0.05:  # 5%+ loss = short
            signals.iloc[i] = -1
        # Else: neutral (0)
...
def run_validation():
    """Run complete momentum strategy validation."""
    print("=" * 70)
    print("PHASE 2: MOMENTUM STRATEGY VALIDATION")
    print("=" * 70)
    print("Academic Jegadeesh-Titman Momentum Strategy")
    print("• 6-month formation period")
    print("• Buy winners (>5% return), short losers (<-5% return)")
    ...
    try:
        ...
        signals = momentum_strategy(market_returns, formation_days=126)
        signal_count = signals.abs().sum()
        long_signals = (signals == 1).sum()
        short_signals = (signals == -1).sum()
        print(f"Generated {signal_count} signals: {long_signals} long, {short_signals} short")
        ...
        if passed >= 4:
            ...
            print("Return to strategy research - momentum may not work in current markets")
